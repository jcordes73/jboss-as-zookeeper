...
public class ZooKeeperParser implements XMLStreamConstants,
    ...
    public void writeContent(XMLExtendedStreamWriter writer, SubsystemMarshallingContext context) throws XMLStreamException {
        ...
        if (node.hasDefined(ZooKeeperExtension.SERVER)) {
            ...
            for (Property property : server.asPropertyList()) {
                ...
                ZooKeeperServerDefinition.TICK_TIME.marshallAsAttribute(entry, true, writer);
                ZooKeeperServerDefinition.DATA_DIR.marshallAsAttribute(entry, true, writer);
                ZooKeeperServerDefinition.SOCKET_BINDING.marshallAsAttribute(entry, true, writer);
            }
    ...
    public void readElement(XMLExtendedStreamReader reader, List<ModelNode> list)
        ...
        PathAddress address = PathAddress.pathAddress(ZooKeeperExtension.SUBSYSTEM_PATH);
        ModelNode addSubsystemOperation = Util.createAddOperation(address);
        ...
        if (reader.hasNext() && reader.nextTag() != END_ELEMENT) {
            ...
            readServer(reader, list, address);
        }
    }

    private void readServer(XMLExtendedStreamReader reader, List<ModelNode> list, PathAddress address)
            throws XMLStreamException {
        ModelNode addServerOperation = Util.createAddOperation();
        ...
        for (int i = 0; i < reader.getAttributeCount(); i++) {
            ...
            if (attr.equals("name")) {
                ...
                ZooKeeperServerDefinition.TICK_TIME.parseAndSetParameter(value, addServerOperation, reader);
            } else if (attr.equals(ZooKeeperExtension.DATA_DIR)) {
                ZooKeeperServerDefinition.DATA_DIR.parseAndSetParameter(value, addServerOperation, reader);
            } else if (attr.equals(ZooKeeperExtension.SOCKET_BINDING)) {
                ZooKeeperServerDefinition.SOCKET_BINDING.parseAndSetParameter(value, addServerOperation, reader);
            } else {
        ...
        PathAddress addr = address.append(ZooKeeperExtension.SERVER, serverName);
